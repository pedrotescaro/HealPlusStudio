...
export function AgendaView() {
  ...
  const handleAddAppointment = async () => {
    if (!user || !newAppointment.date || !newAppointment.patientName || !firestore) {
      toast({
    ...
    try {
      ...
      const docRef = await addDoc(collection(firestore, "users", user.uid, "appointments"), appointmentData);
  ...
  const updateAppointmentStatus = async (appointmentId: string, status: Appointment['status']) => {
    ...
    try {
      ...
      await updateDoc(doc(firestore, "users", user.uid, "appointments", appointmentId), {
        status,
