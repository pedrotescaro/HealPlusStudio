...
export function ReportGenerator() {
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      ...
      try {
        const q = query(collection(firestore, "users", user.uid, "anamnesis"), orderBy("data_consulta", "desc"));
  ...
  const handleVisionMock = async () => {
    if (!selectedRecord || !firestore) return;
    try {
      setLoading(true);
      ...
      if (user) {
        ...
        await addDoc(collection(firestore, "users", user.uid, "assessments"), {
          anamnesisId: selectedAnamnesisId,
  ...
  const handleFhirPush = async () => {
    if (!selectedAnamnesisId || !firestore) return;
    try {
      ...
      if (user) {
        await addDoc(collection(firestore, "users", user.uid, "fhirLogs"), {
    ...
    try {
        ...
        autoTable(doc_, {
            ...
            body: [
                ...
                [`Profissional Responsável: ${(user as any).name || 'Não informado'}`],
        ...
        autoTable(doc_, {
            startY: finalY,
            head: [['Relatório (Análise Estática)']],
            body: [[report.report.replace(/## |### |-/g, '')]],
            theme: 'grid'
        });
